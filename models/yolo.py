...
class BaseModel(nn.Module):
    ...
    def _forward_once(self, x, profile=False, visualize=False):
        y, dt = [], []  # outputs
        #canny_in=x
        ### Canny Edge detection ### and apending making 2,4,H,W input
       # mag, edge = canny(x)
       # x=torch.cat((x,edge), 1)
        for m in self.model:
            ...
            #try:
                #LOGGER.info(f"{x.shape} input to ")
            #except:
                 #LOGGER.info(f"Skip")
...
class DetectionModel(BaseModel):
    ...
    def __init__(self, cfg='yolo.yaml', ch=3, nc=None, anchors=None):  # model, input channels, number of classes
        ...
        if anchors:
            LOGGER.info(f'Again Overriding model.yaml anchors with anchors={anchors}')
...
def parse_model(d, ch):  # model_dict, input_channels(3)
    ...
    #c1, c2 chanel in and chanel out
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        if m in {
            ...
            RepNCSPELAN4, SPPELAN,depthwise_separable_conv,CBAM,Involution2d,eca_layer}:
            c1, c2 = ch[f], args[0]
